...
CustomFakeVideoCapturer::CustomFakeVideoCapturer(bool is_screencast)
    : running_(false),
...
CustomFakeVideoCapturer::CustomFakeVideoCapturer() : CustomFakeVideoCapturer(false) {}

CustomFakeVideoCapturer::~CustomFakeVideoCapturer() {
  SignalDestroyed(this);
}

void CustomFakeVideoCapturer::ResetSupportedFormats(
    const std::vector<cricket::VideoFormat>& formats) {
  SetSupportedFormats(formats);
}

bool CustomFakeVideoCapturer::CaptureFrame() {
  if (!GetCaptureFormat()) {
    return false;
  }
...
bool CustomFakeVideoCapturer::CaptureCustomFrame(int width, int height) {
  // Default to 30fps.
...
bool CustomFakeVideoCapturer::CaptureFrame(const webrtc::VideoFrame& frame) {
  if (!running_) {
    return false;
  }
...
cricket::CaptureState CustomFakeVideoCapturer::Start(
...
void CustomFakeVideoCapturer::Stop() {
  running_ = false;
  SetCaptureFormat(NULL);
  SetCaptureState(cricket::CS_STOPPED);
}

bool CustomFakeVideoCapturer::IsRunning() {
  return running_;
}

bool CustomFakeVideoCapturer::IsScreencast() const {
  return is_screencast_;
}

bool CustomFakeVideoCapturer::GetPreferredFourccs(std::vector<uint32_t>* fourccs) {
  fourccs->push_back(cricket::FOURCC_I420);
  fourccs->push_back(cricket::FOURCC_MJPG);
  return true;
}

void CustomFakeVideoCapturer::SetRotation(webrtc::VideoRotation rotation) {
  rotation_ = rotation;
  if (frame_source_)
    frame_source_->SetRotation(rotation_);
}

webrtc::VideoRotation CustomFakeVideoCapturer::GetRotation() {
  return rotation_;
}

FakeVideoCapturerWithTaskQueue::FakeVideoCapturerWithTaskQueue(
    bool is_screencast)
    : CustomFakeVideoCapturer(is_screencast) {}

FakeVideoCapturerWithTaskQueue::FakeVideoCapturerWithTaskQueue() {}

bool FakeVideoCapturerWithTaskQueue::CaptureFrame() {
  if (task_queue_.IsCurrent())
    return CustomFakeVideoCapturer::CaptureFrame();
  bool ret = false;
  task_queue_.SendTask(
      [this, &ret]() { ret = CustomFakeVideoCapturer::CaptureFrame(); });
  return ret;
}

bool FakeVideoCapturerWithTaskQueue::CaptureCustomFrame(int width, int height) {
  if (task_queue_.IsCurrent())
    return CustomFakeVideoCapturer::CaptureCustomFrame(width, height);
  bool ret = false;
  task_queue_.SendTask([this, &ret, width, height]() {
    ret = CustomFakeVideoCapturer::CaptureCustomFrame(width, height);
  });
  return ret;
}
