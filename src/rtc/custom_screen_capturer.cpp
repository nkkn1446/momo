...
CustomScreenCapturer::CustomScreenCapturer() {
...
bool CustomScreenCapturer::Init(const DesktopCaptureOptions& options) {
  TRACE_EVENT0("webrtc", "CustomScreenCapturer::Init");
  options_ = options;
...
void CustomScreenCapturer::InitXDamage() {
...
void CustomScreenCapturer::Start(Callback* callback) {
...
void CustomScreenCapturer::CaptureFrame() {
  TRACE_EVENT0("webrtc", "CustomScreenCapturer::CaptureFrame");
...
bool CustomScreenCapturer::GetSourceList(SourceList* sources) {
...
bool CustomScreenCapturer::SelectSource(SourceId id) {
  // TODO(jiayl): implement screen selection.
  return true;
}

bool CustomScreenCapturer::HandleXEvent(const XEvent& event) {
...
std::unique_ptr<DesktopFrame> CustomScreenCapturer::CaptureScreen() {
...
void CustomScreenCapturer::ScreenConfigurationChanged() {
  TRACE_EVENT0("webrtc", "CustomScreenCapturer::ScreenConfigurationChanged");
...
void CustomScreenCapturer::SynchronizeFrame() {
...
void CustomScreenCapturer::DeinitXlib() {
  if (gc_) {
    XFreeGC(display(), gc_);
    gc_ = nullptr;
  }
...
// static
std::unique_ptr<DesktopCapturer> CustomScreenCapturer::CreateRawScreenCapturer(
    const DesktopCaptureOptions& options) {
  if (!options.x_display())
    return nullptr;

  std::unique_ptr<CustomScreenCapturer> capturer(new CustomScreenCapturer());
