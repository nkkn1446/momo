...
CustomWindowCapturer::CustomWindowCapturer(const DesktopCaptureOptions& options)
...
CustomWindowCapturer::~CustomWindowCapturer() {
  x_display_->RemoveEventHandler(ConfigureNotify, this);
}

bool CustomWindowCapturer::GetSourceList(SourceList* sources) {
...
bool CustomWindowCapturer::SelectSource(SourceId id) {
...
bool CustomWindowCapturer::FocusOnSelectedSource() {
  if (!selected_window_)
    return false;
...
void CustomWindowCapturer::Start(Callback* callback) {
...
void CustomWindowCapturer::CaptureFrame() {
  TRACE_EVENT0("webrtc", "CustomWindowCapturer::CaptureFrame");
  ...
  auto window_size = x_server_pixel_buffer_.window_size();
  // TODO:ピクセルサイズ指定
  window_size.set(window_size.width(), window_size.height());
  std::unique_ptr<DesktopFrame> frame(
      new BasicDesktopFrame(window_size));
...
bool CustomWindowCapturer::IsOccluded(const DesktopVector& pos) {
  return window_finder_.GetWindowUnderPoint(pos) !=
         static_cast<WindowId>(selected_window_);
}

bool CustomWindowCapturer::HandleXEvent(const XEvent& event) {
...
bool CustomWindowCapturer::GetWindowTitle(::Window window, std::string* title) {
  int status;
  bool result = false;
...
// static
std::unique_ptr<DesktopCapturer> CustomWindowCapturer::CreateRawWindowCapturer(
    const DesktopCaptureOptions& options) {
  if (!options.x_display())
    return nullptr;
  return std::unique_ptr<DesktopCapturer>(new CustomWindowCapturer(options));
}
